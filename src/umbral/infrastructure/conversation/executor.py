...
class EffectExecutor:
    ...
    def _create_radar(
        self, command: CreateRadarCommand, context: TurnContext
    ) -> ExecutedAct:
    ...
    def _set_filter(
        self, command: SetFilterCommand, context: TurnContext
    ) -> ExecutedAct:
    ...
    def _clear_filter(
        self, command: ClearFilterCommand, context: TurnContext
    ) -> ExecutedAct:
        profile_id = _profile_id(context)
        if profile_id is None:
            return ExecutedAct(
    ...
    def _record_desire(
        self, command: RecordDesireCommand, context: TurnContext
    ) -> ExecutedAct:
        profile_id = _profile_id(context)
        if profile_id is None:
            return ExecutedAct(
    ...
    def _revise_desire(
        self, command: ReviseDesireCommand, context: TurnContext
    ) -> ExecutedAct:
        profile_id = _profile_id(context)
        if profile_id is None:
            return ExecutedAct(
    ...
    def _withdraw_desire(
        self, command: WithdrawDesireCommand, context: TurnContext
    ) -> ExecutedAct:
        profile_id = _profile_id(context)
        if profile_id is None:
            return ExecutedAct(
    ...
    def _binding_drafts(
        self, concept_links: tuple[ConceptLink, ...]
...
def _profile_id(context: TurnContext) -> UUID | None:
...
def _current_value(context: TurnContext, filter_key: str) -> object | None:
