...
class ContextAssembler:
    ...
    def load(
        ...
        *,
        ...
        return TurnContext(
            ...
            focused_entity=(
                FocusedEntity(entity_ref=listing.entity_ref)
    ...
    ) -> tuple[str | None, int | None, tuple[HardFilter, ...]]:
    ...
    def _desire_views(self, *, profile_id: UUID | None) -> tuple[DesireView, ...]:
...
def _filters_from_profile(profile: SearchProfile) -> tuple[HardFilter, ...]:
    filters: list[HardFilter] = []
    if profile.zones:
        filters.append(HardFilter(filter_key="zones", value=tuple(profile.zones)))
    if profile.budget_max is not None:
        filters.append(HardFilter(filter_key="budget_max", value=profile.budget_max))
    if profile.min_rooms is not None:
        filters.append(HardFilter(filter_key="min_rooms", value=profile.min_rooms))
    return tuple(filters)


def _desire_view(view: PreferenceView) -> DesireView:
