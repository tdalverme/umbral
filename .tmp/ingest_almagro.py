...
raw_bytes = pathlib.Path(".data/almagro.json").read_bytes()
...
if existing:
    print(f"exists {existing.run_id} {existing.state}")
    run_id = existing.run_id
else:
    run_id = uuid.uuid4()
    now = datetime.now(timezone.utc)
    run = run_repo.create(run_id=run_id, source=source, batch_key=batch_key, file_format="json", file_name="almagro.json", file_sha256=batch_key, file_size_bytes=len(raw_bytes), raw_storage_key=f"objects/raw/{batch_key}", job_execution_id=None, correlation_id=uuid.uuid4(), actor_kind="operator", actor_id="ops-cli", now=now)
...
print(f"silver total={summary.total_snapshots} inserted={summary.listings_inserted} skipped={summary.skipped}")
...
with engine.connect() as c:
    for r in c.execute(text("select neighborhood, geo_precision, geo_source, count(*) from silver_listings group by 1,2,3 order by count(*) desc")):
        print(r)
