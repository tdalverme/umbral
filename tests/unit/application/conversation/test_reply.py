...
def _composer(gateway: ModelGateway) -> ReplyComposer:
...
def test_invalid_model_output_falls_back_deterministically() -> None:
    ...
    reply = composer.compose(
        _result(outcomes=(ActOutcome("a1", "applied"),))
    )
...
def test_pending_hard_step_asks_one_question_with_ordinal() -> None:
    ...
    context = replace(
        context,
        pending_action=PendingAction("pending:zones", "zones", 1, 2),
    )
...
def test_resolution_describes_outcome_then_asks_next_hard_step(
    ...
    context = replace(
        context,
        pending_action=PendingAction("pending:budget", "budget", 2, 2),
    )
    ...
    reply = composer.compose(
        _result(
            ...
            outcomes=(
                ActOutcome("zones", "pending", "filter.requires_confirmation"),
                ActOutcome(
                    "resolve:zones:1",
                    cast(Any, status),
                    reason_code,
