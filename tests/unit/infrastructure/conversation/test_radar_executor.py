...
def _executor(radar_ctx: RadarTestContext, chat: ChatService) -> EffectExecutor:
...
def test_rejecting_pending_returns_a_rejected_resolution_outcome() -> None:
    ...
    context = replace(
        context,
        pending_action=PendingAction(pending_ref=f"pending:{proposal.proposal_id}"),
    )

    result = ProposalsPendingResolver(proposals=proposals).resolve(
        act_id="resolve:a1",
        context=context,
...
def test_clear_filter_creates_pending_proposal_and_keeps_state() -> None:
    ...
    context = _context(
        profile_id=profile.profile_id,
        version=profile.version,
        filters=(HardFilter(filter_key="zones", value=("palermo",)),),
        user_id=user_id,
        session_id=uuid4(),
    )
