...
def test_request_endpoint_is_neutral_for_unknown_email() -> None:
    ...
    response = client.post(
        ...
        headers={
            "X-Umbral-BFF-Token": _deps().settings.bff_token,
            "X-Umbral-Origin-Fingerprint": "bff",
        },
    ...
    assert (
        ...
        == "Si la dirección está habilitada, recibirás un enlace para continuar."
    )
...
def test_capture_confirmation_never_accepts_get_or_returns_token() -> None:
    ...
    assert "token_hash" in str(
        document["components"]["schemas"]["MagicLinkConfirmation"]
    )
