...
def _login() -> tuple[IdentityAccess, AccessControl, InMemoryIdentityStore, str, datetime]:
    ...
    access.request_magic_link(email="person@example.com", origin_fingerprint="o", correlation_id=uuid4(), now=now)
    ...
    session = access.confirm_magic_link(attempt_id=attempt.id, token_hash=str(token_hash), now=now)
...
def test_role_and_ownership_are_rechecked_on_each_operation() -> None:
    ...
    user_id = user.id
    assert control.authorize(token, action="product.resource.read", resource_owner_id=user_id, now=now).user_id == user_id
    with pytest.raises(IdentityError) as error:
        control.authorize(token, action="product.resource.read", resource_owner_id=uuid4(), now=now)
    assert error.value.code == "auth.access_denied"
    AccessAdministration(store).set_user_status(user_id, status="disabled", now=now + timedelta(minutes=1))
    with pytest.raises(IdentityError):
        control.authorize(token, action="auth.session.read", resource_owner_id=None, now=now + timedelta(minutes=2))
