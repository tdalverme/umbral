...
- Retención: 35 días con lock de retención.
- RTO máximo: 4 horas, restaurando beside-primary.
- El bucket de objetos de Railway es privado; no se publican URLs ni ACLs.
...
1. Crear un dump lógico cifrado y copiar las nuevas versiones inmutables al
   bucket de objetos de Railway bajo `backups/preview/`.
...
`umbral.ops.restore.restore_backup`. El adapter remoto (`backup-preview.ps1`)
conserva las mismas salidas y escribe contra el bucket único S3 de Railway;
el drill remoto requiere el CLI `aws` y `pg_dump` y no forma parte del check
local.
