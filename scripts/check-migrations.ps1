...
try {
    ...
    & $pythonPath -c "from umbral.infrastructure.db.migrations import expected_schema; expected={'job_executions','job_attempts','job_outbox_messages','job_schedules','stored_objects','stored_object_versions','runtime_surface_status','identity_invitations','product_users','external_identity_links','role_assignments','magic_link_requests','magic_link_attempts','product_sessions','access_audit_events','import_runs','raw_listing_snapshots','quarantine_records','canonical_properties','silver_listings','dedupe_links','listing_changes','search_profiles','search_profile_versions','recommendation_runs','recommendation_items','product_events','concepts','concept_versions','criterion_bindings','preference_facts','preference_expressions','profile_criteria_compilations','extraction_versions','listing_observations','recomputation_runs','listing_embeddings','urban_categories','urban_contracts','urban_primitives','urban_signals','urban_snapshots','neighborhood_signal_stats','scoring_policies','scoring_policy_versions','criterion_evaluations','recommendation_narratives','comparison_shortlists','feedback_events','feedback_event_reasons','learning_policies','learning_policy_versions','learning_proposals','chat_sessions','chat_messages','agent_graph_runs','agent_node_runs','agent_model_calls','search_profile_update_proposals','agent_eval_suites','agent_eval_case_results','notification_preferences','notification_decisions','notification_inbox_items'}; actual=set(expected_schema().tables); assert actual == expected, (actual ^ expected)"
    if ($LASTEXITCODE -ne 0) {
