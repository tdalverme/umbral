...
try {
    ...
    $conversationSurface = @(
        (Join-Path $repoRoot "src\umbral\application\conversation"),
        (Join-Path $repoRoot "contracts\agent\interpretation-schema.json"),
        (Join-Path $repoRoot "tests\contract\test_agent_contracts.py")
    ) | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
        ...
        if (Test-Path -LiteralPath $pythonExe) {
            try {
                ...
                & $pythonExe -m pytest @(
                    ...
                    (Join-Path $repoRoot "tests\unit\application\conversation"),
                    (Join-Path $repoRoot "tests\unit\agent\intent\test_interpretation.py"),
                    (Join-Path $repoRoot "tests\unit\agent\test_graph.py"),
                    (Join-Path $repoRoot "tests\unit\application\agent_evals\v4")
                ) -q
                if ($LASTEXITCODE -ne 0) {
                    throw ("La suite Conversation termino con codigo {0}." -f $LASTEXITCODE)
                }
                Write-Host "[PASS] Conversation" -ForegroundColor Green
            }
            catch {
                Write-Host ("[FAIL] Conversation: {0}" -f $_.Exception.Message) -ForegroundColor Red
                $script:failures++
            }
        }
        else {
            Write-Host "[SKIP] Conversation: no existe .venv\Scripts\python.exe."
        }
    }
    else {
        Write-Host "[SKIP] Conversation: no existe la superficie conversacional."
    }
