...
function Invoke-Aws([string[]]$Arguments) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "R2 recovery operation failed." }
    return $output
}

foreach ($name in @("DATABASE_MIGRATION_URL", "R2_RECOVERY_BUCKET", "OBJECT_STORE_ENDPOINT_URL", "R2_RECOVERY_ACCESS_KEY_ID", "R2_RECOVERY_SECRET_ACCESS_KEY")) {
...
if (-not (Get-Command aws -ErrorAction SilentlyContinue)) { throw "AWS CLI is required for R2 recovery backup." }
...
try {
    $env:AWS_ACCESS_KEY_ID = $env:R2_RECOVERY_ACCESS_KEY_ID
    $env:AWS_SECRET_ACCESS_KEY = $env:R2_RECOVERY_SECRET_ACCESS_KEY
    $env:AWS_EC2_METADATA_DISABLED = "true"

    & pg_dump --format=custom --file $temporaryDump $env:DATABASE_MIGRATION_URL 2>$null
    if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $temporaryDump)) { throw "Neon backup failed." }
    ...
    Invoke-Aws @("--endpoint-url", $env:OBJECT_STORE_ENDPOINT_URL, "s3api", "put-object", "--bucket", $env:R2_RECOVERY_BUCKET, "--key", $dumpKey, "--body", $temporaryDump, "--metadata", ("sha256={0},backup-id={1}" -f $dumpHash, $backupId)) | Out-Null
    Invoke-Aws @("--endpoint-url", $env:OBJECT_STORE_ENDPOINT_URL, "s3api", "put-object", "--bucket", $env:R2_RECOVERY_BUCKET, "--key", $manifestKey, "--body", $temporaryManifest) | Out-Null
    $head = (Invoke-Aws @("--endpoint-url", $env:OBJECT_STORE_ENDPOINT_URL, "s3api", "head-object", "--bucket", $env:R2_RECOVERY_BUCKET, "--key", $dumpKey) | Out-String | ConvertFrom-Json)
    if ([int64]$head.ContentLength -ne $dumpSize -or [string]$head.Metadata.sha256 -ne $dumpHash) { throw "R2 backup verification failed." }
